...
CustomChown() {
    ...
    while [ "$Q_LOOP1" = true ] ; do
        read -p "OWNERNAME?File owner name: "
        read -p "GROUPNAME?File group name: "
        chown $OWNERNAME:$GROUPNAME $SCRIPTNAME
        if [ "$?" != "0" ] ; then
            clear
...
CustomChmod() {
    ...
    while [ "$Q_LOOP2" = true ] ; do
        read -p "OWNERRIGHTS?Owner rights code: "
        read -p "GROUPRIGHTS?Group rights code: "
        read -p "OTHERSRIGHTS?Others rights code: "
